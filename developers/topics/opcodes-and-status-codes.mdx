...
description: Complete reference for all Discord Gateway opcodes, voice opcodes, HTTP response codes, JSON error codes, and RPC error/close codes.
---
...
To prevent broken reconnect loops, treat certain close codes as a signal to stop reconnecting. This table explains all application-defined close codes for the gateway and indicates which should not be retried.

<Warning>
Close codes `4004`, `4010`, `4011`, `4012`, `4013`, and `4014` indicate permanent configuration problems. **Do not attempt to reconnect** after receiving these codes — fix the underlying issue first.
</Warning>

| Code | Description           | Explanation                                                                                                                                                          | Reconnect |
|------|-----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|
| 4000 | Unknown error         | We're not sure what went wrong. Try reconnecting?                                                                                                                    | true      |
| 4001 | Unknown opcode        | You sent an invalid Gateway opcode or an invalid payload for an opcode.                                                                                              | true      |
| 4002 | Decode error          | You sent an invalid payload to Discord.                                                                                                                              | true      |
| 4003 | Not authenticated     | You sent a payload prior to identifying, or this session has been invalidated.                                                                                       | true      |
...
| 4008 | Rate limited          | You're sending payloads too quickly. Slow it down! You will be disconnected on receiving this.                                                                       | true      |
| 4009 | Session timed out     | Your session timed out. Reconnect and start a new one.                                                                                                               | true      |
| 4010 | Invalid shard         | You sent an invalid shard when identifying.                                                                                                                          | false     |
...
Discord's voice gateways have their own set of opcodes and close codes, separate from the main Gateway.

### Voice Opcodes

Opcodes marked in the **Binary** column are transmitted as binary WebSocket messages rather than JSON text. See the [Voice Connections](/developers/topics/voice-connections) page for the binary message format.
...
| 23   | DAVE Transition Ready               | client            | Acknowledge readiness for a previously announced transition. |   |
| 24   | DAVE Prepare Epoch                  | server            | A DAVE protocol version or group change is upcoming.     |        |
| 25   | DAVE MLS External Sender            | server            | Credential and public key for MLS external sender.       | ✓      |
| 26   | DAVE MLS Key Package                | client            | MLS Key Package for pending group member.                | ✓      |
| 27   | DAVE MLS Proposals                  | server            | MLS Proposals to be appended or revoked.                 | ✓      |
| 28   | DAVE MLS Commit Welcome             | client            | MLS Commit with optional MLS Welcome messages.           | ✓      |
| 29   | DAVE MLS Announce Commit Transition | server            | MLS Commit to be processed for upcoming transition.      | ✓      |
| 30   | DAVE MLS Welcome                    | server            | MLS Welcome to group for upcoming transition.            | ✓      |
...
| 4002 | Failed to decode payload      | You sent an invalid payload in your identifying request to the Gateway.                                             |
...
| 4014 | Disconnected                  | Disconnect (you were kicked, the main gateway session was dropped, etc.). **Should not reconnect.**                 |
| 4015 | Voice server crashed          | The server crashed. Try resuming.                                                                                   |
| 4016 | Unknown encryption mode       | We didn't recognize your encryption mode.                                                                           |
...
| 4021 | Disconnected: Rate Limited    | Disconnected due to rate limit exceeded. **Should not reconnect.**                                                  |
| 4022 | Disconnected: Call Terminated | All clients disconnected because the call was terminated (channel deleted, voice server changed, etc.). **Should not reconnect.** |

---

## HTTP

The Discord API returns semantically valid HTTP response codes based on the success of your request.
...
| 502 (GATEWAY UNAVAILABLE) | There was not a gateway available to process your request. Wait and retry.     |
...
Along with the HTTP error code, the API returns detailed error codes through a `code` key in the JSON error response. The response also contains a `message` key with a human-readable error string. Some errors include additional details in an `errors` object.

<Tip>
Always check both the HTTP status code and the JSON `code` field when handling errors — the JSON code gives you the specific reason, while the HTTP code gives you the category.
</Tip>
...
### JSON Error Codes Table

<Accordion title="10xxx — Unknown Resource Errors">
...
<Accordion title="20xxx — Action Restriction Errors">
...
<Accordion title="30xxx — Limit Reached Errors">
...
<Accordion title="40xxx — Request State Errors">
...
<Accordion title="50xxx — Invalid Request Errors">
...
<Accordion title="60xxx–530xxx — Miscellaneous Errors">
...
| 530003 | The audience in the ID token JWT you have provided does not match the audience you specified in your OIDC configuration   |
| 530004 | The ID token you provided was issued too long ago. Discord will not accept ID tokens issued beyond a week ago             |
