...
- **Entry Point commands:** used to launch [Activities](/developers/activities/overview) from the App Launcher
...
This model works well for apps that only need to respond to commands and UI components. No persistent connection is required and it scales naturally with standard web infrastructure. To use it, your endpoint must validate Discord's request signatures and handle the initial `PING` handshake.

<Info>
  See the [Interactions Overview](/developers/interactions/overview#preparing-for-interactions) for endpoint setup details including signature validation.
</Info>
